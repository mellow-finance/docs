...
| Operation-level risk         | Permissions are external or vault-boundary-only | 60+ onchain permission types scoped by strategy, asset, operator, action. Unauthorized transactions cannot be executed                                                        |
...
* **DeFi access:** see the [canonical integration list](/mellow-vaults-overview#execution-venues) for supported venues
