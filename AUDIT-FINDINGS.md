...
| index.mdx | 10 | Core Vaults are the primary architecture for current structured product deployments, while other vault types cover specialized use cases such as DVT-o | rewrite | **Correction (manual review):** this is the exact sentence the source spec's Section 4.2 names as a known finding — it gives the legacy stack present-tense, equal billing on the most-crawled page. Replace with one subordinating sentence pointing at the historical-architectures section of `/mellow-vaults-overview`, and drop "LRT" from the homepage entirely. Originally misclassified `flag`; reclassified `rewrite` on manual reconciliation. |
...
## 3a. Supplementary findings (manual reconciliation against source spec Section 4)

The items below were confirmed by manually re-checking the delegated grep audit above against the source spec's Section 4 (known page-level findings). They fall outside all fifteen Section 3 grep patterns — mostly because they're typos, phrase-consistency issues, or structural/title issues rather than lexical pattern matches — so they were not caught automatically. Audit-only; not fixed here.

| Path | Line | Issue | Class | Note |
|---|---|---|---|---|
| index.mdx | 10 | "Core Vaults are the **primary architecture** for current structured product deployments..." | rewrite | Violates Section 2's phrase-standardization rule: use one phrase site-wide, "Core Vaults are Mellow's current vault architecture." Never alternate "primary"/"current"/"flagship". |
| index.mdx | 14 | Card body: "Mellow's **primary architecture** for curated onchain structured products." | rewrite | Same phrase-standardization violation. |
| core-vaults/overview.mdx | 6 | "Core Vaults are Mellow's **primary vault architecture** for deploying curated onchain structured products." | rewrite | Same violation — this is the exact "primary" instance named in source spec Section 4.4, contrasted there against `mellow-vaults-overview.mdx`'s "current architecture" phrasing (line 12, already correct-ish but not the exact mandated sentence either). |
| vault-infrastructure-for-fintech-earn-products.mdx | 26 | "Core Vaults are the **primary vault type** for institutional and partner deployments..." | rewrite | Same violation, third distinct variant phrase. |
| mellow-vaults-overview.mdx | 12, 124 | "Core Vaults are Mellow's **current architecture** for new deployments." / "Core Vaults are the **current architecture**." | rewrite | Closest existing phrasing to the mandated sentence but not an exact match either — needs to become the literal "Core Vaults are Mellow's current vault architecture." everywhere, including here. |
| core-vaults/architecture/index.mdx | 447 | `IERC20.tranfer,               // transfer call encoding` (code sample) | fix | Named explicitly in source spec Section 4.3 as an unambiguous typo: `IERC20.tranfer` → `IERC20.transfer`. In a code block but spec explicitly carves out this one typo as a "Do not touch...except the two named typos in Section 4.3" exception. |
| core-vaults/architecture/index.mdx | 450, 469 | `// extra data for Verifer contract` (code sample comments, 2 occurrences) | fix | Named explicitly in source spec Section 4.3: "Verifer contract" → "Verifier contract". Same code-block exception as above. |
| core-vaults/architecture/index.mdx | 393, 436, 438, 454 | "ByBit" (x2), "B\*\*ybit Clearloop" (garbled), "Bybit UI" — inconsistent capitalization of the same venue in prose | fix | Source spec Section 4.3: normalize "ByBit"/"B\*\*ybit" to "Bybit" in prose. Line 438 in particular ("B\*\*ybit Clearloop") looks like corrupted/mis-pasted text, not just a capitalization issue — worth a close look, not a mechanical find-replace. |
| mellow-vaults-overview.mdx | 35 | "More than 60 operation-level onchain permissions" | flag | This is the exact phrase source spec Section 4.5 names as a load-bearing, unowned figure needing an Andrey decision — the primary instance. (A second, differently-worded instance — "60+ onchain permission types" — was already noted at `core-vaults-and-other-approaches.mdx:54` under Section 4.1 below; both should be reconciled to the same figure once confirmed.) |
| resources/mellow-lrt-depreciated/index.mdx | 2 | `title: "Mellow-LRT (Depreciated)"` | fix | Frontmatter title typo: "Depreciated" should be "Deprecated". Note: this typo is NOT in the `docs.json` nav label (which currently reads "LRT V1.0" — see Section 1 discrepancy note) but survives in this page's own frontmatter title. Also note this page's opening line ("Mellow LRT is a restaking primitive designed to operate within the dynamic environment of the restaking ecosystem.") has already been partially cleaned up from the wording the source spec quotes ("Mellow Protocol is an innovative liquid restaking protocol...AVS ecosystem") but is still present tense and still lacks the legacy banner and `noindex` the spec's 4.7 exception calls for. |
...
- **"More than 60 operation-level onchain permissions" figure** — found once: `core-vaults-and-other-approaches.mdx` line 54, "60+ onchain permission types scoped by strategy, asset, operator, action." Confirm this figure is current/accurate. **RESOLVED 2026-09-16 (Andrey): "current is fine"** — the figure is accurate as-is. The four slightly-different phrasings across `core-vaults-and-other-approaches.mdx:54,107`, `mellow-vaults-overview.mdx:35`, and `vault-infrastructure-for-fintech-earn-products.mdx:34` were not unified into a single canonical sentence — only the accuracy question was asked and answered; standardizing the wording is a separate, still-open cleanup if wanted.
