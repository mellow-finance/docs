...
The `Subvault` contract represents a modular, permissioned vault component designed to manage delegated asset strategies within a parent `Vault`. It enables curated logic for permissioned calls and delegated strategy execution without exposing external deposit or redemption interfaces.
