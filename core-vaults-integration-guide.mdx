...
A **Mellow Core Vault** is a programmable, modular vault contract. It serves as the central hub for capital management, risk control, and composable logic. **Depositors** provide capital; **Curators** manage that capital within guardrails set by the vault configuration.
...
| `InsufficientBalance`    | Both         | Token or share balance too low                                                        | Validate balance onchain before submitting                                         |
...
Listen for these events to drive UI state or index onchain activity.
